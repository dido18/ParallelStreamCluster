...
#endif
    // centerIDs and IDOFFEST can be eliminated
...
struct lastStage:ff_minode_t<Points> { // NOTE multi-input node
  ...
  //    cluster_centers.p[i].weight = 1.0;
    }
  ...
  // printPoints(centers);
    ...
    for(int i=0; i<centers->num; ++i){
      ...
      cluster_centers->p[counter+i].ID = centers->p[i].ID;
      cluster_centers->p[counter+i].cost = centers->p[i].cost;
      for(int c=0; c < centers->dim ; ++c)
	cluster_centers->p[counter+i].coord[c] = centers->p[i].coord[c];
    }
    counter +=centers->num;
    cluster_centers->num = counter;
