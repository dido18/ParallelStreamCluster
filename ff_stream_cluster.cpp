...
/* declaration of C functions of PARSEC */
...
void outcenterIDs( Points* centers,  char* outfile ) {
  ...
  for( int i = 0; i < centers->num; i++ ) {
    if( is_a_median[i] ) {
      //fprintf(fp, "%u\n", centerIDs[i]);
      fprintf(fp, "weigth: %lf \ncoordinates: ", centers->p[i].weight);
...
struct lastStage:ff_minode_t<Points> { // NOTE multi-input node
  ...
  Points* svc(Points * centers){
    ...
    outcenterIDs(centers, "output.txt"); // DIFFERENTE DA ORIGIBNALE
    delete centers;
    return GO_ON;
...
int main (int argc, char *argv[]){
  ...
  EmitterChunks emitter(stream, chunksize, clustersize);   
